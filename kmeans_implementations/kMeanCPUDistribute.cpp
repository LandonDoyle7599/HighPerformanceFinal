...
void kMeanDistributePerformance(int argc, char*argv[],int epochs, int k, std::string fileName, std::string output_dir){
  ...
  if(myRank == 0){
    ...
    srand(time(0));
    for(int i = 0; i < k; i++){
      globalCX[i] = globalArrX[rand() % n];
      globalCY[i] = globalArrY[rand() % n];
      globalCZ[i] = globalArrZ[rand() % n];
    }
...
void kMeanDistribute(int argc, char*argv[],int epochs, int k, std::string fileName, std::string output_dir){
  ...
  if(myRank == 0){
    while(std::getline(file,line)){
      ...
      for(int i = 0; i < k; i++){
        int index = rand() % n;
        globalCX[i] = globalArrX[index];
        globalCY[i] = globalArrY[index];
        globalCZ[i] = globalArrZ[index];
      }
  }
  MPI_Bcast(&n, 1, MPI_INT, 0, MPI_COMM_WORLD);
  MPI_Bcast(globalCX.data(), k, MPI_DOUBLE, 0, MPI_COMM_WORLD);
